...
void drawShape()
{
  ...
  //beginShape();
  //vertex(0-2*d,y);
    for (int x = -2*(d) ;x < width+2*d; x+=inc)
    {
     a2 = abs((x-width/3)); //map(a2,0,width/2,1,d); //bigger in the middle left
     //tshoot(x); 
     //line(x+t ,y +curveY(0,false),(x)+d+t,y -+curveY(0,true));
      line(x+t ,y + curveY(0,false),x+t,y - curveY(0,true));
     // vertex((x)+(inc/2)+t,y - curveY(0,true));
