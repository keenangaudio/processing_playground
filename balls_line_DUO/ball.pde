class part
{
  ...
  part(int size, float speed)
  {
    ...
    rad*=size;
    mass= rad*rad*0.05;
    col.set(random(20,255),random(20,255),random(20,255));
  ...
  void bounce(part b2)
  {
    float dist = loc.dist(b2.loc), 
          ...
          d.set(-cos(angle)*(Tmome-(momentum/mass)),  //X
                -sin(angle)*(Tmome-(momentum/mass))); //Y
  }
